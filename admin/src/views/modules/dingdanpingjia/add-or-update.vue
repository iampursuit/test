<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="水果名称" prop="shuiguomingcheng">
          <el-select  @change="shuiguomingchengChange" v-model="ruleForm.shuiguomingcheng" placeholder="请选择水果名称">
            <el-option
                v-for="(item,index) in shuiguomingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.shuiguomingcheng" label="水果名称" prop="shuiguomingcheng">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shuiguoxinxi/shuiguomingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shuiguomingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    shuiguomingchengChange () {
      this.$http({
        url: `follow/shuiguoxinxi/shuiguomingcheng?columnValue=`+ this.ruleForm.shuiguomingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.shuiguofenlei){
                this.ruleForm.shuiguofenlei = data.data.shuiguofenlei
              }
        } else {
          this.$message.error(data.msg);
        }
